...
export default fastify => ({
    ...
    async handler(req, rep) {
        ...
        try {
            ...
            const count = await this.mongo.db.collection('boats').find(query, options).count();
            ...
            const boats = (await this.mongo.db.collection('boats').find(query, options).toArray() || []).map(d => {
                const boat = d;
                const [
                    en,
                    ru
                ] = boat.name.split(/\|/);
                boat.title = req.body.language === 'ru' ? ru || en : en;
                return boat;
            });
            // Send response
            return rep.code(200)
