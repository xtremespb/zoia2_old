...
output.on('close', () => {
    ...
    const rsyslogAPI = fs.readFileSync(`${__dirname}/../shared/templates/rsyslog.conf`, 'utf8')
        ...
        .replace(/{filename}/igm, `${site.id}_api.log`)
    ...
    const rsyslogWeb = fs.readFileSync(`${__dirname}/../shared/templates/rsyslog.conf`, 'utf8')
        ...
        .replace(/{filename}/igm, `${site.id}_web.log`)
        .replace(/{path}/igm, path.resolve(`${__dirname}/../logs`).replace(/\\/gm, '/'));
    ...
    console.log(`Writing systemd/${site.id}_api.service...`);
    fs.writeFileSync(`${__dirname}/../dist/config/systemd/${site.id}_api.service`, systemdAPI);
    console.log(`Writing systemd/${site.id}_web.service...`);
    fs.writeFileSync(`${__dirname}/../dist/config/systemd/${site.id}_web.service`, systemdWeb);
    console.log(`Writing rsyslog.d/${site.id}_api.conf...`);
    fs.writeFileSync(`${__dirname}/../dist/config/rsyslog.d/${site.id}_web.conf`, rsyslogAPI);
    console.log(`Writing rsyslog.d/${site.id}_web.conf...`);
    fs.writeFileSync(`${__dirname}/../dist/config/rsyslog.d/${site.id}_api.conf`, rsyslogWeb);
    console.log(`Writing nginx/${site.id}.conf...`);
    fs.writeFileSync(`${__dirname}/../dist/config/nginx/${site.id}.conf`, nginx);
});
